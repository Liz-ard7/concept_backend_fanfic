...
content_id: eb236c57c030bdfb0fb8c955a4f8872e6d1d5c0ea892eb9a64ee25c5e2a985ce
---
...
export default class LibraryConcept {
  ...
  async viewFic(
    ...
    // Using find to respect potential gaps if versions were deleted in a non-sequential manner
  ...
  async deleteFic(
    ...
    // Pull the specific fic from the array
    await this.users.updateOne(
      { _id: user, "versions.title": ficName },
      { $pull: { "versions.$.fics": { versionNumber: versionNumber } } },
    );

    // After pulling, check if the version became empty and delete it if so
    const updatedUserDoc = await this.users.findOne({ _id: user });
    const updatedVersion = updatedUserDoc?.versions.find((v) =>
      v.title === ficName
    );
