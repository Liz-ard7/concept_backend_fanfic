...
content_id: e5a47a3db7cee07f8b0a25b8d93b38fcf94c7abe6c2c7e1620b49dbe7ac947ce
---
...
Deno.test("LibraryConcept", async (t) => {
  ...
  await t.step("should fulfill the principle: submit and view stories", async () => {
    ...
    try {
      ...
      if ("error" in submitFic1Result) throw new Error(submitFic1Result.error);
      ...
      // Type guard
      if ("error" in submitFic2Result) throw new Error(submitFic2Result.error);
      ...
      if ("error" in viewUserVersionsResult) {
        throw new Error(viewUserVersionsResult.error);
      }
