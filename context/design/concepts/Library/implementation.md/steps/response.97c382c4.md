---
timestamp: 'Sun Oct 12 2025 17:00:50 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251012_170050.8f347f5d.md]]'
content_id: 97c382c469ea923a5b058cdc8b7fcc23ac2066633d4b49bdae1b88e6188b7b04
---
 ...
 */
interface Fic {
  _id: ID; // Unique ID for this specific fic revision, internally managed.
  name: string; // The name of the story, typically matches the parent Version's title
  text: string;
  authorTags: string[]; // Set of strings
  date: DateSpec;
  versionNumber: number; // 0-indexed revision number within its Version
}
...
interface Version {
  ...
  fics: Fic[]; // All revisions of this story, ordered by versionNumber implicitly
}
...
export default class LibraryConcept {
  ...
  constructor(private readonly db: Db) {
    // The main collection representing the "set of Users"
  ...
  async submitNewFic(
    ...
    // Precondition: ficName must not exist as a version title for this user
    if (userDoc.versions.some((v) => v.title === ficName)) {
      return { error: `Fic with name '${ficName}' already exists for user '${user}'.` };
    }

    const newFic: Fic = {
      _id: freshID(), // Generate a unique ID for this specific fic revision
      name: ficName,
      text: ficText,
      authorTags: authorTags,
      date: date,
      versionNumber: 0, // First version is always 0
    };
    ...
    // Effect: Add the new version to the user's set of Versions
  ...
  async submitNewVersionOfFanfic(
    { user, ficText, authorTags, versionTitle, date, ficName }: {
      ...
      versionTitle: string;
      date: DateSpec;
      ficName: string; // Should match versionTitle for consistency and searchability
    },
    ...
    // Precondition: versionTitle must exist for this user
    if (versionIndex === -1) {
      return {
    ...
    // Calculate the next versionNumber as the current number of fics (0-indexed)
    const newVersionNumber = targetVersion.fics.length;

    const newFic: Fic = {
      _id: freshID(),
      name: ficName, // Should match versionTitle as per definition
      text: ficText,
      authorTags: authorTags,
      date: date,
    ...
    // Effect: Add the new Fic to the existing version's set of Fics
    await this.users.updateOne(
      { _id: user, "versions.title": versionTitle }, // Find the user and the specific version by title
      { $push: { "versions.$.fics": newFic } }, // Push the new fic into the 'fics' array of the matched version
    );

    // Fetch the updated userDoc to return the full updated version (MongoDB doesn't return the full doc on $push)
    ...
    if (!updatedVersion) {
      ...
      return { error: "Failed to retrieve updated version after submission." };
    }
   ...
   * @param {string} ficName The title of the story (version).
  ...
  async viewFic(
    ...
    // Precondition: ficName must exist as a version title for this user
    if (!targetVersion) {
      return {
    ...
    // Precondition: versionNumber must be within valid range
    if (
      versionNumber < 0 || versionNumber >= targetVersion.fics.length
    ) {
      return {
        error:
          `Version number '${versionNumber}' is out of range for fic '${ficName}'. Valid range is 0 to ${targetVersion.fics.length - 1}.`,
      };
    }
    ...
    // This check is mostly redundant if versionNumber is in range and versions.fics are properly maintained
    if (!fic) {
      return {
        error:
          `Fic revision with version number '${versionNumber}' not found for '${ficName}'. (Internal error, should not happen)`,
      };
    }
   ...
   *          If deleting the last fic in a version, the version itself is also removed.
   */
  async deleteFic(
    { user, ficName, versionNumber }: {
      user: User;
    ...
    // Precondition: ficName must exist as a version title
    if (!version) {
      return {
        error: `Fic with name '${ficName}' does not exist for user '${user}'.`,
      };
    ...
    // Precondition: versionNumber must exist within the range
    if (!ficToDelete) {
      return {
        error:
    ...
    // Effect: Remove the fic from the version's fics array
    // This removes the fic identified by its _id within the nested array
    await this.users.updateOne(
      { _id: user, "versions.title": ficName },
      { $pull: { "versions.$.fics": { _id: ficToDelete._id } } }, // Use _id for reliable pull
    );

    // After deleting a fic, we need to re-index the remaining version numbers
    // and potentially delete the version if it becomes empty.
    const updatedUserDocAfterPull = await this.users.findOne({ _id: user });
    const updatedVersion = updatedUserDocAfterPull?.versions.find((v) =>
      v.title === ficName
    );

    if (updatedVersion) {
      if (updatedVersion.fics.length === 0) {
        ...
        // Re-index version numbers of remaining fics
        // Sort by original versionNumber to ensure consistent re-indexing order
        const reIndexedFics = updatedVersion.fics.sort((a, b) =>
          a.versionNumber - b.versionNumber
        ).map((fic, index) => ({
          ...fic,
          versionNumber: index, // Assign new sequential version numbers
        }));

        // Update the 'fics' array for the specific version
        await this.users.updateOne(
          { _id: user, "versions.title": ficName },
          { $set: { "versions.$.fics": reIndexedFics } },
        );
      }
    }

    return { fic: ficToDelete }; // Return the original fic object that was deleted
  }
  ...
  async deleteFicsAndUser(
    ...
    // Effect: Remove the user document completely
  ...
  async findFicWithDate(
    ...
    // Effect: Iterate through all versions and their fics to find matches
  ...
  async deleteVersion(
    ...
    // Precondition: ficTitle must exist as a version title
    if (!versionToDelete) {
      return {
    ...
    // Effect: Remove the entire version object from the user's versions array
    await this.users.updateOne(
      { _id: user },
   ...
   * @description Retrieves all story versions for a given user. This is a non-mutating query.
   * @param {User} user The ID of the user.
