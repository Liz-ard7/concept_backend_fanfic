...
Deno.test("Principle: User submits stories and views them in their library.", async () => {
  ...
  try {
    ...
    const submitFic1Result = await libraryConcept.submitNewFic({
      ...
      authorTags: ["fantasy", "adventure"],
      date: dateJan1_2023,
    });
    assertNotEquals("error" in submitFic1Result, true, "First fic submission should succeed.");
    const fic1 = (submitFic1Result as { fic: { _id: ID } }).fic;
    assertExists(fic1._id);
    console.log(`submitNewFic (user: ${userAlice}, ficName: "The Enchanted Forest", ...): { fic: ${fic1._id} }`);
    ...
    const submitFic2Result = await libraryConcept.submitNewFic({
      ...
      authorTags: ["scifi", "space opera"],
      date: dateJan2_2023,
    });
    assertNotEquals("error" in submitFic2Result, true, "Second fic submission should succeed.");
    const fic2 = (submitFic2Result as { fic: { _id: ID } }).fic;
    assertExists(fic2._id);
    console.log(`submitNewFic (user: ${userAlice}, ficName: "Starship Odyssey", ...): { fic: ${fic2._id} }`);
...
Deno.test("Scenario: Submitting new versions and verifying updates.", async () => {
  ...
  try {
    ...
    const ficV0 = (submitFicResult as { fic: { _id: ID } }).fic;
    console.log(`submitNewFic (user: ${userBob}, ficName: "Epic Tale", ...): { fic: ${ficV0._id} }`);
    ...
    const viewFicV0Result = await libraryConcept.viewFic({
      user: userBob,
      ficName: "Epic Tale",
      versionNumber: 0,
    });
    ...
    const viewFicV1Result = await libraryConcept.viewFic({
      user: userBob,
      ficName: "Epic Tale",
      versionNumber: 1,
    });
    ...
    const getVersionResult = await libraryConcept.getVersion({
      user: userBob,
      versionTitle: "Epic Tale",
    });
...
Deno.test("Scenario: Error cases for fic submission and version updates.", async () => {
  ...
  try {
    ...
    const invalidVersionNumberResult = await libraryConcept.viewFic({
      user: userCharlie,
      ficName: "Unique Story",
      versionNumber: 99,
    });
...
Deno.test("Scenario: findFicWithDate and deleteFicsAndUser.", async () => {
  ...
  try {
    ...
    let findJan1Result = await libraryConcept.findFicWithDate({
      user: userEve,
      date: dateJan1_2023,
    });
    ...
    let findJan2Result = await libraryConcept.findFicWithDate({
      user: userEve,
      date: dateJan2_2023,
    });
    ...
    const findNonExistentDateResult = await libraryConcept.findFicWithDate({
      user: userEve,
